...
{
	public sealed class FilteredGenerator<T> : IDiscreteGenerator<T>
	{
		[NotNull] private IDiscreteGenerator<T> m_filteredGenerator;
		[NotNull] private IFilter<T>[] m_filters;

		private T[] m_sequence;
		private byte m_currentSequenceLength;

		public FilteredGenerator([NotNull] IDiscreteGenerator<T> filteredGenerator,
			[NotNull] params IFilter<T>[] filters)
		{
		...
		public FilteredGenerator([NotNull] FilteredGenerator<T> other)
		{
