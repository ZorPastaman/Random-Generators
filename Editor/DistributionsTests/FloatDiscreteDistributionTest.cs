...
{
	/// <summary>
	/// Editor window for testing <see cref="IDiscreteGenerator{T}"/> of type <see langword="float"/>.
	/// </summary>
	public sealed class FloatDiscreteDistributionTest : EditorWindow
	{
		private ObjectField m_floatProviderView;
		...
		private readonly List<(float, uint)> m_generatedCounts = new();
		private uint m_generations;

		private Keyframe[] m_keyframes;

		[MenuItem("Window/Random Generators/Float Discrete Distribution Test", priority = 2021)]
		public static void OpenWindow()
		{
			var window = GetWindow<FloatDiscreteDistributionTest>("Float Discrete Distribution Test");
		...
		private void CreateGUI()
		{
			...
			m_floatProviderView = new ObjectField("Float Discrete Generator Provider") {objectType = typeof(DiscreteGeneratorProvider<float>)};
			root.Add(m_floatProviderView);
		...
		{
			var generatorProvider = m_floatProviderView.value as DiscreteGeneratorProvider<float>;
		...
		private void Update()
		{
			...
			for (uint generation = 0u; generation < m_generationsPerFrame; ++generation)
			{
				...
				if (index >= 0)
				{
					(float valueIndex, uint generatedCount) = m_generatedCounts[index];
		...
		private int FindGeneratedIndex(float valueIndex)
		{
