...
namespace Zor.RandomGenerators.DiscreteDistributions.DistributionFilters
{
	public sealed class SamePatternFilter<T> : IFilter<T>
	{
		...
		{
			byte requireSequenceLength = (byte)(m_maxPatternLength * 2 + 1);

			if (sequenceLength < requireSequenceLength ||
			...
			for (int i = sequenceLength - requireSequenceLength, j = sequenceLength - m_maxPatternLength;
